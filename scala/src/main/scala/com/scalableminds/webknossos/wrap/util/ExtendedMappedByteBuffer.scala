...
class ExtendedMappedByteBuffer(mappedData: MappedByteBuffer) {
  ...
  def copyTo(offset: java.lang.Integer, other: Array[Byte], destPos: java.lang.Integer, length: java.lang.Integer): Box[Unit] = {
  ...
  def copyFrom(offset: java.lang.Integer, other: Array[Byte], srcPos: java.lang.Integer, length: java.lang.Integer): Box[Unit] = {
    // Any regularly called log statements in here should be avoided as they drastically slow down this method.
    if (offset + length <= mappedData.limit()) {
      tryo {
        for (i <- 0 until length) {

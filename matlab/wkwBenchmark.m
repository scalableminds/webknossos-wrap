function wkwBenchmark(param)
    ...
    for curSizeIdx = 1:sizeCount
        ...
        for curRep = 1:repCount
            ...
            % wk-wrap (raw)
            tic;
            wkwLoad(wkwFile, curSize, curRoi(:, 1)', cls);
            data(curSizeIdx, 2, curRep) = toc;

            % wk-wrap (compressed)
            if curSize >= 32
                tic;
                wkwLoad(wkwCompFile, curSize, curRoi(:, 1)', cls);
    ...
    legend( ...
        'KNOSSOS (128 voxels)', ...
        'WK-Wrap (1024 voxels, raw)', ...
        'WK-Wrap (1024 voxels, LZ4-HC compressed)', ...
        'HDF5 (1024 voxels, uncompressed)', ...
        'HDF5 (1024 voxels, deflate compressed)', ...
        'Location', 'NorthWest');
end

function [wkwFile, wkDir, cls, sz, testFilename1, testFilename2] = buildData(param)
    cls = 'uint32';
    sz = 1024;
    ...
    % save wk-wrap file
    disp('<< Writing raw wk-wrap file...');
    wkwFile = fullfile(benchDir, 'wk-wrap.raw');
    wkwSave(wkwFile, data, [1, 1, 1]);

    disp('<< Compressing wk-wrap file...');
    wkwCompress(wkwFile, strcat(wkwFile, '.lz4'));

...
function boxes = wkwSplitFileBox(box, cubeIds, minSize, cubeSize)
    ...
    boxes = wkwSplitCubeBox(relBox, maxBox, minSize);
    boxes = bsxfun(@plus, boxes, relOff);
end

function boxes = wkwSplitCubeBox(box, maxBox, minSize)
    ...
    elseif any(box(:) ~= maxBox(:)) && any(diff(maxBox, 1, 2) > minSize)
        ...
        boxes = cat(3, ...
            wkwSplitCubeBox(box, newBoxMake([0, 0, 0]), minSize), ...
            wkwSplitCubeBox(box, newBoxMake([0, 0, 1]), minSize), ...
            wkwSplitCubeBox(box, newBoxMake([0, 1, 0]), minSize), ...
            wkwSplitCubeBox(box, newBoxMake([0, 1, 1]), minSize), ...
            wkwSplitCubeBox(box, newBoxMake([1, 0, 0]), minSize), ...
            wkwSplitCubeBox(box, newBoxMake([1, 0, 1]), minSize), ...
            wkwSplitCubeBox(box, newBoxMake([1, 1, 0]), minSize), ...
            wkwSplitCubeBox(box, newBoxMake([1, 1, 1]), minSize));
    else
        % perfect fit
        boxes = maxBox;
    end
